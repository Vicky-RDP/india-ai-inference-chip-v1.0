...
export default function Home() {
  return (
    <main>
      ...
      <section className="architecture section shell"><div className="architecture-intro"><div className="section-kicker">03 / IAIC v1.0</div><h2>The beginning<br />of the stack.</h2><p>IAIC v1.0 starts narrow on purpose: an open, verifiable INT8 inference core that can move from simulation to FPGA to silicon. Then the community expands the stack around it.</p><a className="text-link dark-link" href={roadmapUrl} target="_blank" rel="noreferrer">Read the technical roadmap <span aria-hidden="true">↗</span></a></div><div className="stack-grid">{stack.map(([number, title, copy]) => <article className="stack-card" key={number}><span>{number}</span><div><h3>{title}</h3><p>{copy}</p></div></article>)}</div></section>
